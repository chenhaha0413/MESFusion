...
def extract_features_video_images(video_images, image_size, k, **kwargs):
    ...
    for clean_video_image_index in range(len(video_images) - k):
        ...
        optical_flow_create = cv2.DualTVL1OpticalFlow_create() #Depends on cv2 version
...
def preprocess(clean_videos_images, clean_videos_images_features, k, image_size=128):
    predictor_model = "/home/y/pytorch-expression-spotting/__utils__/shape_predictor_68_face_landmarks.dat"
    face_detector = dlib.get_frontal_face_detector()
    face_pose_predictor = dlib.shape_predictor(predictor_model)
